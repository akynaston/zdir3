...
; for modiyfing MyShortCuts-swa.ahk window to get into focus
; didn't work - july 20 2013 #WinActivateForce

CURRENTTIMESHEETID=28973
...
ObisidianBinary=C:\Users\x266698\AppData\Local\Obsidian\obsidian.exe
NPP = "C:\Program Files\Notepad++\notepad++.exe"
SHADOWVM=C:\shadow0\vmware
MYSHORTCUTSAHK=c:\work\adminrepo\MyShortCuts-swa.ahk
...
;
; This hotkey runs when Scroll Lock is pressed, but only if it's currently ON
;ScrollLock::
;    if GetKeyState("ScrollLock", "T") {
;        Send, {Esc} ; Example action when pressed while ON
;    }
;return
...
msgbox "uh oh - outside of main functions"
...
doEditMainScripts()
{
  ...
  {
      WinActivate
      WinWaitActive %MYSHORTCUTSAHK%
  }
  else
  {
      Run, "C:\Program Files\Notepad++\notepad++.exe" c:\work\adminrepo\MyShortCuts-swa.ahk
      WinActivate
      WinWaitActive MyShortCuts-swa.ahk
  }
  WinMove MyShortCuts-swa.ahk, , , , 1400, 1000
  ControlFocus
  CenterWindow("MyShortCuts-swa.ahk")
...
^!+e::
  ...
  MsgBox , , , Obsidian loading . ., .25

  IfWinActive Obsidian
  {
    ; allow window to be toggled!
    WinMinimize
    WinHide
    return
  }
...
;[[Twiki|https://wiki.trivir.com/]]
;akynaston: akyn@st0n

; Run %BROWSERCHOICE% https://wiki.trivir.com/dev/qanda
;Run %BROWSERCHOICE% https://wiki.trivir.com/doku.php?id=dev:start
Run %BROWSERCHOICE% https://trivir.atlassian.net/wiki/spaces/TRIVIR/overview

;this link is fcps's jira portal:
;https://trivir.atlassian.net/servicedesk/customer/portal/1
...
{
    Run, %NPP% C:\work\adminrepo\workNotes.txt
    WinActivate
    WinWaitActive workNotes.txt
}
...
^!+t::
  ...
  WinShow TriVirTimes.ods

  IfWinActive TriVirTimes.ods
  {
    ; allow window to be toggled!
      WinMinimize
      WinHide
  ...
  ; othewise, lets open it up, and show:
  ...
  WinMove TriVirTimes.ods, , %xExpected%, %yExpected%, %xExpectedWidth%, %yExpectedHeight%
...
{
    Run, %NPP% C:\work\adminrepo\zplate.txt
    Run, %NPP% C:\work\adminrepo\zstandup.txt
    WinActivate
    WinWaitActive zstandup.txt
}
...
;====================================================================
^!+p::
; See https://www.autohotkey.com/docs/v1/misc/WinTitle.htm#ahk_class for ahk_class!
  IfWinActive PingID ahk_class GlassWndClass-GlassWindowClass-2
  {
    ; allow window to be toggled!
    WinMinimize
    WinHide
  ...
  MsgBox , , , Ping id requested to start up . ., .25
